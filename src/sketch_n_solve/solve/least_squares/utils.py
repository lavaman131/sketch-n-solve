...
def lsqr(
    A: Union[sparse.spmatrix, np.ndarray, LinearOperator],
    b: np.ndarray,
    damp: float = 0.0,
    atol: float = 1e-6,
    btol: float = 1e-6,
...
) -> Tuple[np.ndarray, int, int, float, float, float, float, float, float, np.ndarray]:
    ...
    if arnorm == 0:
        if show:
            print(msg[0])
        return x, istop, itn, r1norm, r2norm, anorm, acond, arnorm, xnorm, var  # type: ignore
    ...
    while itn < iter_lim:  # type: ignore
        ...
        if show:
            ...
            if itn <= 10:
                prnt = True
            if itn >= iter_lim - 10:  # type: ignore
                prnt = True
